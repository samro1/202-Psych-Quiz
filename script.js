...
function loadQuestion() {
  ...
  answeredCurrent = userAnswers[currentQuestionIndex] !== undefined;
  ...
  q.answers.forEach((answer, index) => {
    ...
    input.name = 'answer';
    input.value = index;
    input.disabled = answeredCurrent;

    const label = document.createElement('label');
    label.htmlFor = `answer${index}`;
    label.textContent = answer.text;

    // Mark checked and apply styles if already answered
    if (userAnswers[currentQuestionIndex] === index) {
      input.checked = true;
      if (answer.correct) label.classList.add('correct-answer');
...
function showExplanation() {
  const q = shuffledQuestions[currentQuestionIndex];
  const chosenIndex = userAnswers[currentQuestionIndex];
  const chosenAnswer = q.answers[chosenIndex];

  const explanationDiv = document.createElement('div');
  explanationDiv.className = 'explanation';
  explanationDiv.innerHTML = `${chosenAnswer.correct ? "<span style='color:green;font-weight:bold;'>Correct!</span>" : "<span style='color:red;font-weight:bold;'>Incorrect.</span>"} ${q.explanation}`;
...
nextBtn.addEventListener('click', () => {
  if (!answeredCurrent) {
    ...
    if (selected) {
      ...
      loadQuestion(); // Reload to lock answers and show explanation
      nextBtn.textContent = currentQuestionIndex === shuffledQuestions.length - 1 ? "Finish" : "Continue";
    }
  } else {
